...
class HomeFragment : Fragment() {
    ...
    private lateinit var popularThisSeasonAdapter: PopularThisSeasonRvAdapter
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        popularThisSeasonAdapter = assignPopularThisSeasonRvAdapter(viewModel.popularThisSeasonList)
        popularThisSeasonRecyclerView.adapter = popularThisSeasonAdapter
    ...
    private fun setupObserver() {
        ...
        viewModel.popularThisSeasonData.observe(viewLifecycleOwner, Observer {
            when (it.responseStatus) {
                ResponseStatus.LOADING -> popularThisSeasonLoading.visibility = View.VISIBLE
                ResponseStatus.SUCCESS -> {
                    popularThisSeasonLoading.visibility = View.GONE
                    if (it.data?.Page()?.media()?.isNullOrEmpty() == false) {
                        viewModel.popularThisSeasonList.clear()
                        it.data.Page()?.media()?.forEach { media ->
                            viewModel.popularThisSeasonList.add(media)
                        }
                        popularThisSeasonRecyclerView.adapter?.notifyDataSetChanged()
                    }
                }
                ResponseStatus.ERROR -> {
                    popularThisSeasonLoading.visibility = View.GONE
    ...
    private fun assignPopularThisSeasonRvAdapter(list: List<PopularSeasonQuery.Medium>): PopularThisSeasonRvAdapter {
        val metrics = DisplayMetrics()
        activity?.windowManager?.defaultDisplay?.getMetrics(metrics)
        val width = metrics.widthPixels / 3
        return PopularThisSeasonRvAdapter(activity!!, list, width, object : PopularThisSeasonRvAdapter.PopularThisSeasonListener {
            override fun passSelectedAnime(mediaId: Int) {
                val intent = Intent(activity, BrowseActivity::class.java)
                intent.putExtra(BrowseActivity.TARGET_PAGE, BrowsePage.ANIME.name)
                intent.putExtra(BrowseActivity.LOAD_ID, mediaId)
